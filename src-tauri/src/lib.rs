...
fn apply_split_routing(cfg: &mut serde_json::Value, split: &SplitRoutingSettings) {
	...
	if !bypass_paths.is_empty() {
		new_rules.push(json!({ "process_path": bypass_paths, "outbound": split.direct_outbound }));
	}
	if !bypass_names.is_empty() {
		new_rules.push(json!({ "process_name": bypass_names, "outbound": split.direct_outbound }));
	}

	let (proxy_names, proxy_paths) = split_process_tokens(&split.proxy_apps);
	if !proxy_paths.is_empty() {
		new_rules.push(json!({ "process_path": proxy_paths, "outbound": split.proxy_outbound }));
	}
	if !proxy_names.is_empty() {
		new_rules.push(json!({ "process_name": proxy_names, "outbound": split.proxy_outbound }));
	}

	// domains -> direct/proxy
	...
	if !bypass_domains.is_empty() {
		new_rules
			.push(json!({ "domain_suffix": bypass_domains, "outbound": split.direct_outbound }));
	}
	...
	if !proxy_domains.is_empty() {
		new_rules.push(json!({ "domain_suffix": proxy_domains, "outbound": split.proxy_outbound }));
	}
