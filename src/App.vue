...
export default defineComponent({
  ...
  methods: {
    ...
    async openLogs() {
      // вариант 1: Rust-команда open_logs (если сделаешь)
      // await invoke('open_logs')

      // вариант 2: открыть файл по пути (если ты знаешь путь)
      // здесь оставлю заглушку — лучше сделать команду на Rust, чтобы вернуть путь к log файлу
      try {
        await open('https://admin.ultunnel.ru') // временно чтобы проверить работу
      } catch (e: any) {
        this.errorText = String(e)
      }
    },
...
.v { color: rgba(255,255,255,.95); }

.muted { color: rgba(255,255,255,.65); font-size: 13px; padding: 8px 0; }
.error { margin-top: 10px; color: #ff8a8a; font-size: 13px; }
...
.fab.on { background: #b48cff; color: #111; }
...
.nav-ico { font-size: 18px; line-height: 18px; }
